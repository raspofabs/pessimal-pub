...
class Field:
    ...
    def read(self, destination, config):
        ...
        if string_value in [None, "None"]:
            ...
            try:
                ...
                print(
                    f"Error when parsing {self.name} : {self.__class__.__name__}/{self.default_value}"
                )
    ...
    def edit(self, component):  # pragma: no cover
        current_value = self.get_value(component)
        if current_value is None:
            current_value = "None"
        changed, new_value = imgui.input_text(
            self.name, current_value, imgui.INPUT_TEXT_ENTER_RETURNS_TRUE
        )
        if changed:
    ...
    def edit(self, component):  # pragma: no cover
        ...
        if self.config.get("slider"):
            ...
            changed, new_value = imgui.slider_int(
                self.name, current_value, min_value, max_value
            )
        else:
...
class V2Field(Field):
    ...
    def edit(self, component):  # pragma: no cover
        current_value = self.get_value(component)
        changed, new_values = imgui.input_float2(
            self.name, current_value.x, current_value.y
        )
        if changed:
